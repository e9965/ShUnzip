...
#Search for the zip file
  ...
  do
    case "${filename##*.}" in
    "zip")filelist[$i]=${filename}
       let "i++"
    ;;
    "7z")filelist[$i]=${filename}
       let "i++"
    ;;
    "rar")filelist[$i]=${filename}
       let "i++"
    ;;
	esac	
  done
...
#Start to use Passwd
        ...
        do
          TryPassUnzip "${passwd[`expr ${SizeArray} - ${c}`]}" ${filelist[fc]} "${Opzip}"
          if [ $? == 0 ]
            then
                let "unzipsucc++"
